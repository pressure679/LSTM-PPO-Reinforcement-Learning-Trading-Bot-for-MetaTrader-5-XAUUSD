    ...
    try:
        while True:
            ...
            for i in range(SEQ_LEN, len(df)):
                ...
                # Start of a new week (save_counter is a multiple of
                # save_count, including the very first iteration) —
                # preload this week's state_seq rows as one vectorized
                # slice instead of hitting df.iloc[i] per bar.
                ...
                high = current["High"]
                low = current["Low"]
                # base_sl = min(50, df["volume"].rolling(20).mean().iloc[i])
                # sl_scale = min(1.0, max(MIN_SL_SCALE, current_rf / TARGET_RF))
                # base_sl = BASE_SL_PIPS * sl_scale
            ...
            if open_pos == 0:
                ...
                if action == 1:
                    ...
                    risk_per_position = min(
                        max((balance * 0.0025) / (TP_PIPS * 10), 0.01),
                        100.0
                    )
