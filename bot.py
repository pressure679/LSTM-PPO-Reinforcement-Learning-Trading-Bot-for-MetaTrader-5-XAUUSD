...
def load_last_mb_xauusd(file_path=None, mb=19, delimiter=';', col_names=None):
...
def add_indicators(df, include_higher_tf=True):
    ...
    # EACH 1m ROW
    # ==================================================================
    # Right-labeled/left-closed resample so e.g. a 5m bin covering
    # [10:00, 10:05) is labeled 10:05 — i.e. it only becomes visible once
    ...
    if include_higher_tf:
        ...
        for prefix, freq in (("5m", "5min"), ("15m", "15min"), ("30m", "30min"), ("1h", "1h")):
...
def train_bot(symbol="XAUUSD"):
    ...
    # itself) — also what gets "5m_"/"15m_"-prefixed for the merged higher-tf candle
    # indicators, since add_indicators() runs this same pipeline on the
    # resampled 5m/15m/30m/1h data and prefixes its output with "5m_"/"15m_"/"30m_"/"1h_" respectively.
    ...
    FEATURES = (
        ...
        + ["30m_" + f for f in BASE_INDICATOR_FEATURES]
        + ["1h_" + f for f in BASE_INDICATOR_FEATURES]
...
def get_today_profit(symbol, day, magic=123456):
    ...
    deals = mt5.history_deals_get(day_start, datetime.now() + timedelta(days=1))
...
def test_bot(symbol="XAUUSD"):
    ...
    # The rolling live window needs enough 1m bars that resampling all
    # the way up to 1h still leaves a comfortable margin over the
    # largest indicator lookback (72 bars) — 12000 1m bars ≈ 200 1h
    # bars ≈ 8.3 days (the 5m/15m branches get much bigger margins
    # still, ~2400/~800 bars, from this same window). Too small a
    # window here would make the 1h (or 15m/5m) branch come back empty
    # and silently wipe out every row via add_indicators()'s dropna().
    ...
    while True:
        ...
        if current_m1 != last_m1:
            ...
            # 1R = day_start_balance * RISK and the target is 5R.
            today_profit = get_today_profit(symbol, last_trading_date)
            # profit_target_ratio = today_profit / (5 * (day_start_balance * RISK))
            profit_target_ratio = today_profit / ((1 / RISK) * 5 * (day_start_balance * RISK))

            if profit_target_ratio >= 1:
                if open_pos != 0:
                    close_trades()
